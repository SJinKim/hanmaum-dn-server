...
class GroupMeetingService(
    ...
    fun createMeeting(req: CreateMeetingRequest): Long {
        val group = groupRepo.findById(req.groupId)
    ...
    @Transactional
    fun submitReport(meetingId: Long, req: SubmitMeetingReportRequest) {
        ...
        val attendances = req.entries.map { entry ->
            val member = memberRepo.findByPublicId(UUID.fromString(entry.memberId))
    ...
    fun getMeetingDetails(meetingId: Long, requesterMemberId: String, isAdminOrPastor: Boolean): MeetingDetailDto {
        val meeting = meetingRepo.findById(meetingId)
            .orElseThrow { EntityNotFoundException("Meeting not found") }
        ...
        if (!isAdminOrPastor) {
            val requester = memberRepo.findByPublicId(UUID.fromString(requesterMemberId))
    ...
    @Transactional(readOnly = true)
    fun getMeetings(requesterMemberId: String, isAdminOrPastor: Boolean): List<GroupMeetingDto> {
        val meetings = if (isAdminOrPastor) {
